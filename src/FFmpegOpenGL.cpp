...
FFmpegOpenGL::FFmpegOpenGL(QWidget* parent)
...
void FFmpegOpenGL::setUrl(QString url)
{
    decoder->setUrl(url);
}

void FFmpegOpenGL::startVideo()
{
    decoder->resume();
    if (!decoder->isRunning()) {
        decoder->start();
...
void FFmpegOpenGL::stop()
{
    decoder->stop();
}

void FFmpegOpenGL::initializeGL()
{
...
void FFmpegOpenGL::resizeGL(int w, int h)
{
    if (h <= 0) h = 1;
